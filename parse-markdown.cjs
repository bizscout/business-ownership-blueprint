...
archetypes.forEach((archetype, index) => {
  ...
  if (sectionStart !== -1) {
    ...
    variations.forEach(v => {
        const oppMatch = v.match(/\*\*Biggest Opportunity:\*\*\s*(.*?)(?=\n\n\*\*|$)/s);
        const mistMatch = v.match(/\*\*Most Common Mistake:\*\*\s*(.*?)(?=\n\n\*\*|$)/s);
        const focusMatch = v.match(/\*\*Your 90-Day Focus:\*\*\s*(.*?)(?=\n\n\*\*|$)/s);
        const questionMatch = v.match(/\*\*One Question to Sit With:\*\*\s*\*(.*?)\*(?=\n\n|$)/s);
