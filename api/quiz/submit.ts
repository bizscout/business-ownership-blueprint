...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!parsed.success) {
    console.log("[quiz] Validation failed for", req.body?.email ?? "unknown", "| errors:", JSON.stringify(parsed.error.flatten().fieldErrors), "| answers:", req.body?.answers?.length ?? "missing");
